...
@section('content')
    ...
    <form action="{{ route('ketqua.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="form-row">
            <div class="form-group col-md-6">
                <label for="ttsv">Thông tin sinh viên</label>
                <select id="ttsv" class="form-control selectpicker" name="sinhvien_id" data-live-search="true">
                    @foreach ($sinhvien as $item)
                        <option value="{{ $item->id }}" {{ old('sinhvien_id') == $item->id ? 'selected' : '' }}>
        ...
        <div class="form-row">
            <div class="form-group col-md-3">
                ...
                <input type="text" class="form-control" id="dth" placeholder="Nhập điểm tin học"
                    name="diem_tin_hoc" value="{{ old('diem_tin_hoc') }}">
            </div>
            <div class="form-group col-md-3">
                ...
                <input type="text" class="form-control"id="bth" placeholder="Nhập bằng tin học" name="bang_tin_hoc"
                    name="bang_tin_hoc" value="{{ old('bang_tin_hoc') }}">
            </div>
        </div>
        <div class="form-row">
            <div class="form-group col-md-3">
                <label for="dav">Điểm anh văn</label>
                <input type="text" class="form-control" id="dav" placeholder="Nhập điểm tin học"
                    name="diem_anh_van" value="{{ old('diem_anh_van') }}">
            </div>
            <div class="form-group col-md-3">
                <label for="bav">Bằng anh văn</label>
                <input type="text" class="form-control" id="bav" placeholder="Nhập bằng tin học"
                    name="bang_anh_van" value="{{ old('bang_anh_van') }}">
            </div>
        </div>
